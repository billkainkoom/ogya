...
/*class ListableAdapter<T : Listable> internal constructor(
    ...
    fun updateList(newList: MutableList<T>,detectMoves : Boolean = false) {
        QuickAsync(
                initiateAsync = {
                    val diffResult = DiffUtil.calculateDiff(ListableAdapterDiffCallback(this.listables, newList), detectMoves)
                    this.listables.clear()
                    this.listables.addAll(newList)
                    diffResult
                },
                asyncComplete = { diffResult ->
